...
void draw() {
  ...
  for(int l = 0; l<items; l++){
    ...
    if(inMap(fix,fiy)){
      ...
      if(abs(ivx)>8){ivx=abs(ivx)/ivx*8;}
      if(abs(ivy)>8){ivy=abs(ivy)/ivy*8;}

      ix += ivx;
      iy += ivy;
      
      if(ifl==round(ifl)){ifl++;} else {ifl += -1;}

      if(ifl>15*6){ifl=round(ifl)-.5;}
      if(ifl<0){ifl=0;}
...
//ellipse(100,100,40,40);
  ...
  for(int k = 0; k <= maph; k++){
    ...
    line(0,k*40+mof, mapw*40,k*40+mof);

  }
  line(0,maph*40+mof-1, mapw*40,maph*40+mof-1);
  line(mapw*40-1,mof,mapw*40-1,maph*40+mof);
...
boolean iPop(int pitem){
  
  if(gip[pitem]==1){
    int tivx = 0;
    int tivy = 0;
    
    if(grab(gix[pitem],giy[pitem],1,0)){tivx -= 4;}
    if(grab(gix[pitem],giy[pitem],-1,0)){tivx += 4;}
    if(grab(gix[pitem],giy[pitem],0,1)){tivy -= 4;}
    if(grab(gix[pitem],giy[pitem],0,-1)){tivy += 4;}
    
    gix[pitem] = gix[pitem]*40+20+givx[pitem];
    giy[pitem] = giy[pitem]*40+20+mof+givy[pitem];

    gifl[pitem] = 0;
    
    givx[pitem] += random(10)-5+tivx;
    givy[pitem] += random(10)-5+tivy;
...
boolean wrench(int wposx, int wposy){
  if(map[wposx][wposy]>4){
    map[wposx][wposy] = 4;
    int tswtch = floor(random(4));
    if(direct(wposx,wposy,0,0)>=0){
     tswtch = direct(wposx,wposy,0,0);
    }
    map[wposx][wposy] = 5 + tswtch;
  }
  return true;
}
...
boolean resetPoint(int ttitem){
  ...
  gis[ttitem] = 0;
  gifl[ttitem] = 0;
  gix[ttitem] = random(mapw*40-80)+40;
  giy[ttitem] = -40;
